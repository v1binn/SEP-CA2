    ...
    conn.connect(function (err) {
      if (err) {
        ...
        if (password == null || password === "") {
          ...
          var sql = `
            UPDATE memberentity 
            SET NAME=?, PHONE=?, CITY=?, ADDRESS=?, SECURITYQUESTION=?, SECURITYANSWER=?, AGE=?, INCOME=?, SERVICELEVELAGREEMENT=?
            WHERE EMAIL=?
          `;
          ...
          bcrypt.hash(password, 5, function (err, hash) {
            ...
            var sql = `
              UPDATE memberentity 
              SET NAME=?, PHONE=?, CITY=?, ADDRESS=?, SECURITYQUESTION=?, SECURITYANSWER=?, AGE=?, INCOME=?, SERVICELEVELAGREEMENT=?, PASSWORDHASH=?
              WHERE EMAIL=?
            `;
