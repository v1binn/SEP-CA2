        ...
        .then((result) => {
            if (result.success) {
                member.getMember(req.body.email)
                    .then((updatedMember) => {
                        // Send only the updated member object
                        res.status(200).json({
                            success: true,
                    ...
                    .catch((err) => {
                        console.error("Error retrieving updated member:", err);
                        res.status(500).send("Failed to get member");
                    });
            } else {
                // Explicit failure if no rows affected
                res.status(400).send("Update failed: No rows were changed.");
            }
        })
        .catch((err) => {
